  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_0.8fr] gap-4">
      <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-3 sm:p-5">
        ...
        <div className="grid grid-cols-7 text-center text-[9px] sm:text-[11px] uppercase tracking-wide text-slate-500 mb-2">
          ...
          {calendarGrid.map((day) => {
            ...
            return (
              <button
                ...
                className={`calendar-day-btn h-[60px] sm:h-[74px] rounded-lg border px-1 sm:px-1.5 py-1 text-left transition ${
                  selected
                    ? "border-indigo-400 bg-indigo-500/15"
                    : "border-slate-800 bg-slate-950/70 hover:border-slate-700"
                } ${!isCurrentMonth ? "opacity-35" : "opacity-100"}`}
              >
                <p className="calendar-day-number text-[10px] sm:text-[11px] text-slate-200">{day.getDate()}</p>
                {dayData && (
                  <>
                    <div className="calendar-day-stats mt-1 space-y-0.5 hidden sm:block">
                      {dayData.income > 0 && <p className="text-[9px] text-emerald-300 font-bold">+{Math.round(dayData.income)}</p>}
                      {dayData.expense > 0 && <p className="text-[9px] text-red-300 font-bold">-{Math.round(dayData.expense)}</p>}
                    </div>
                    ...
                    <div className="flex gap-1 mt-1 sm:hidden">
                       ...
                       {(dayData.expense > 0) && <div className="w-1 h-1 rounded-full bg-red-400" />}
                    </div>
                  </>
                )}
              </button>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 sm:p-5">
        <h2 className="font-semibold text-white mb-3 text-sm sm:text-base">Details: {selectedDateKey || 'Select Day'}</h2>

        {!selectedDateKey ? (
          <p className="text-sm text-slate-400">Pick a date from the calendar to inspect entries.</p>
        ) : (
          <div className="space-y-2 max-h-[300px] sm:max-h-[500px] overflow-y-auto custom-scrollbar">
                ...
                <div key={txn._id} className="rounded-xl border border-slate-800 bg-slate-950/60 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <p className="font-bold text-white truncate text-xs sm:text-sm">{txn.description}</p>
                      <p className="text-[10px] text-slate-500 mt-0.5 uppercase tracking-tighter">
                        {txn.category} {txn.isSample ? "• Sample" : ""}
                      </p>
                    </div>
                    <p className={`font-black text-xs sm:text-sm whitespace-nowrap ${txn.type === "income" ? "text-emerald-300" : "text-slate-100"}`}>
